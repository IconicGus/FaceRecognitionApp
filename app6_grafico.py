...
def app():
    #input do id da classe
    id_classe = entry1.get()
    ...
    #pega o horario que o programa inicia
    horario_inicial = time.localtime() 

    #Iniciando lista
    n_time = [] #horario do reconhecimento de cada pessoa
    qntd_reconhecimentos_individual = []
    lista_pessoas_prentes = [] #1 se a pessoa estiver presente e 0 se ela não estiver presente
    lista_id_classe = [] #lista para colocar o id da classe
    ...
    for i in range(tamanho_lista_name): #(tamanho_lista_name) => qntd de pessoas na lista de pessoas | tempo do ultimo reconhecimento de cada pessoa
        n_time.append(0)

    for i in range(tamanho_lista_name): #(tamanho_lista_name) => qntd de pessoas na lista de pessoas | tempo do ultimo reconhecimento de cada pessoa
        lista_id_classe.append(id_classe)
    ...
    while True:
            ...
            for i, prob in enumerate(prob_list):
                if prob>0.90:
                    ...
                    if(qntd_reconhecimentos_individual[name_list_idx] >= 20):
                        ...
                        #verifica o horario de ultimo reconhecimento.
                        horario = time.localtime()
                        n_time[int(name_list_idx)] = str(horario[0]) + '-' + str(horario[1]) + '-' + str(horario[2]) + ' | ' + str(horario[3]) + ':' + str(horario[4]) + ':' + str(horario[5]) #guarda o horario do ultimo reconhecimento da peessoa com o id
        ...
        if k%256==27: # ESC
            ...
            matriz = np.array([[name_list], [lista_pessoas_prentes], [n_time], [lista_id_classe]])
            m = np.matrix(matriz)
            #print(m)
            ...
            with open("saida_csv/presenca.csv", "w", newline='') as saida:
                ...
                escrever.writerow(n_time)
                escrever.writerow(lista_id_classe)
...
botao = Button(janela, text="INICIAR", command=app)
botao.grid(column=0, row=2, padx=10, pady=10)

texto_output = Label(janela, text="")
texto_output.grid(column=0, row=3, padx=10, pady=10)

texto_instrucao = Label(janela, text="Para FINALIZAR, aperte ESC na aba da camera!")
texto_instrucao.grid(column=0, row=4, padx=10, pady=10)

texto_instrucao2 = Label(janela, text="Arquivo CSV gerado no diretório: /saida_csv")
texto_instrucao2.grid(column=0, row=5, padx=10, pady=10)
